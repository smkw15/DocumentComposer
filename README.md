...
├─📄.flake8  👈flake8設定ファイル
├─📄config.yml  👈DocumentComposer設定ファイル
...
- ユーザは、コマンドライン引数、または、設定ファイルからこれらの動作を制御する設定値を入力することができます。
...
| `--config`または`-c` | `./config.yaml` | 設定ファイルまでのパス。 |
...
設定ファイルには、DocumentComposerの動作を制御する各種設定情報が定義されています。ユーザはこのファイル定義された以下の設定値を編集することで、DocumentComposerの動作を自由にカスタマイズすることができます。
